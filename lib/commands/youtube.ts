...
export default async (home_scope: HomeScope) => {
	...
	const duration = new Date(res.lengthSeconds * 1000)
		.toISOString()
		.substr(11, 8);
	const views = Number(res.viewCount).to_abbrev(2);
	message.answer(`Search for '${query}' (result №${num}): https://youtu.be/${res.videoId} \n published ${res.publishedText} view count: ${views} duration: ${duration}`);
